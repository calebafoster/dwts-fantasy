...
Modeled after ~/Projects/budget-app: flat structure, no ORM, no migrations.
    ...
    - `templates/` — `login.html` (combined login/first-time-signup form), `index.html` (a player's roster, draft/redraft controls, this week's prediction form), `standings.html` (week-by-week standings from `WeeklyResults`), `admin.html` (add contestants, enter weekly points, finalize the week, season-end bonus).
